...
**`qmen_luming`**（禄命总览）提供完整的六亲禄命解读，支持 4 种视角切换。调用 `qimen_luming.sh` 完成确定性的六亲分配计算，AI 再按六大人生维度（父母/兄弟/子孙/官禄/妻财/疾厄）逐一解读。四种视角：奇门大师（综合解读）、占卜师（占断侧重）、军师（谋略侧重）、法术奇门（仪式侧重）。当用户意图不明时，也作为默认兜底路由。

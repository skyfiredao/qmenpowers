...
脚本会：
- 在 stdout 输出文字盘面（展示给用户看）
- 同时写入 `./qmen_event.json`（后续分析用）

将文字盘面完整展示给用户，简短说明："盘面已生成，正在进行分析……"
...
用 Read 工具读取 `./qmen_analysis.json`（以及命盘分析 JSON，如果有的话），然后按照「解盘框架」生成中文叙事解读。

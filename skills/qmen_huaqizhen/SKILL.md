...
布阵脚本直接读取起局盘面（默认 `./qmen_birth.json`），不依赖财官分析结果。
...
用 Read 工具读取 `./qmen_huaqizhen.json`，按照「布阵解读框架」生成布阵方案。
...
> 布阵完成。当问题解决或不再需要此局时，请封局：起局的纸盖雷霆都司印后封存。动念封局，心中想一位中国英雄（如岳飞、关羽、霍去病、杨靖宇、钱学森、邓稼先），然后删掉遁甲局。
